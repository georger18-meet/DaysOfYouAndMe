	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float4 Layer0VertexOffset605 = ( float4( ( v.normalOS * ( _L0Displacement / 10.0 ) ) , 0.0 ) * temp_output_8_0_g1343 );
				float4 temp_cast_5 = (1.0).xxxx;
				float4 temp_output_10_0_g1344 = ( temp_cast_5 + (-0.5 + (1.0 - 0.0) * (0.5 - -0.5) / (1.0 - 0.0)) );
				float4 Layer1VertexOffset613 = ( float4( ( v.normalOS * ( _L1Displacement1 / 10.0 ) ) , 0.0 ) * temp_output_10_0_g1344 );
						...
						, bool ase_vface : SV_IsFrontFace ) : SV_Target
			{
				...
				float3 worldRefl28_g1354 = normalize( reflect( -WorldViewDirection, float3( dot( tanToWorld0, Normal129 ), dot( tanToWorld1, Normal129 ), dot( tanToWorld2, Normal129 ) ) ) );
				float4 texCUBENode27_g1354 = texCUBElod( _Cubemap, float4( worldRefl28_g1354, _BlurReflection) );
				float clampResult39_g1354 = clamp( _ReflectionIntensity , 0.0 , 100.0 );
				float4 temp_cast_12 = (1.0).xxxx;
				float4 lerpResult704 = lerp( ( BaseColor121 * 0.0 ) , ( ( texCUBENode27_g1354 * ( texCUBENode27_g1354.a * clampResult39_g1354 ) * _ColorCubemap1 ) * temp_cast_12 ) , VertexA117);
				...
				float3 BaseColor = (( _L1VisualizeHeightmap )?( ( Layer1VisHeightmap612 * VertexA117 ) ):( (( _L0VisualizeHeightmap )?( ( Layer0VisHeightmap579 * VertexR114 ) ):( BaseColor121 )) )).xyz;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN , bool ase_vface : SV_IsFrontFace ) : SV_TARGET
			{
				...
				float3 lerpResult133 = lerp( Layer0Normal193 , Layer1Normal260 , ( VertexAHeightmap799 * WetOpacity768 ));
				float3 Normal129 = lerpResult133;
				float3 worldRefl28_g1354 = normalize( reflect( -ase_worldViewDir, float3( dot( tanToWorld0, Normal129 ), dot( tanToWorld1, Normal129 ), dot( tanToWorld2, Normal129 ) ) ) );
				float4 texCUBENode27_g1354 = texCUBElod( _Cubemap, float4( worldRefl28_g1354, _BlurReflection) );
				float clampResult39_g1354 = clamp( _ReflectionIntensity , 0.0 , 100.0 );
				float4 temp_cast_12 = (1.0).xxxx;
				float4 lerpResult704 = lerp( ( BaseColor121 * 0.0 ) , ( ( texCUBENode27_g1354 * ( texCUBENode27_g1354.a * clampResult39_g1354 ) * _ColorCubemap1 ) * temp_cast_12 ) , VertexA117);
