	...
	SubShader
	{
		...
		Pass
		{
			...
			inline float2 ParallaxOffset( half h, half height, half3 viewDir )
			{
				...
				return h* (v.xy / v.z);
					...
					float2 voronoihash58_g1459( float2 p )
					{
					...
					float voronoi58_g1459( float2 v, float time, inout float2 id, inout float2 mr, float smoothness, inout float2 smoothId )
					{
						float2 n = floor( v );
						float2 f = frac( v );
						float F1 = 8.0;
						...
						for ( int j = -1; j <= 1; j++ )
						{
							for ( int i = -1; i <= 1; i++ )
						 	{
						 		float2 g = float2( i, j );
						 		float2 o = voronoihash58_g1459( n + g );
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float4 Layer0VertexOffset605 = ( float4( ( v.normalOS * ( _L0Displacement / 10.0 ) ) , 0.0 ) * temp_output_8_0_g1436 );
				float4 temp_cast_5 = (1.0).xxxx;
				float4 temp_output_10_0_g1435 = ( temp_cast_5 + (-0.5 + (1.0 - 0.0) * (0.5 - -0.5) / (1.0 - 0.0)) );
				float4 Layer1VertexOffset613 = ( float4( ( v.normalOS * ( _L1Displacement1 / 10.0 ) ) , 0.0 ) * temp_output_10_0_g1435 );
						...
						, bool ase_vface : SV_IsFrontFace ) : SV_Target
			{
				...
				float2 texCoord114_g1387 = IN.ase_texcoord8.xyz.xy * temp_cast_3 + float2( 0,0 );
				...
				float2 texCoord105_g1387 = IN.ase_texcoord8.xyz.xy * temp_cast_4 + _OffsetFBDetailsNormal;
				...
				float time58_g1459 = ( 1.0 * 0.001 );
				float2 voronoiSmoothId58_g1459 = 0;
				float2 texCoord55_g1459 = IN.ase_texcoord8.xyz.xy * float2( 1,1 ) + float2( 0,0 );
				float2 coords58_g1459 = texCoord55_g1459 * _Tiling;
				float2 id58_g1459 = 0;
				float2 uv58_g1459 = 0;
				float fade58_g1459 = 0.5;
				float voroi58_g1459 = 0;
				float rest58_g1459 = 0;
				for( int it58_g1459 = 0; it58_g1459 <2; it58_g1459++ ){
				voroi58_g1459 += fade58_g1459 * voronoi58_g1459( coords58_g1459, time58_g1459, id58_g1459, uv58_g1459, 0,voronoiSmoothId58_g1459 );
				rest58_g1459 += fade58_g1459;
				coords58_g1459 *= 2;
				fade58_g1459 *= 0.5;
				}//Voronoi58_g1459
				voroi58_g1459 /= rest58_g1459;
				float2 myVarName60_g1459 = id58_g1459;
				float2 panner63_g1459 = ( 1.0 * _Time.y * temp_cast_19 + ( step( voroi58_g1459 , 0.1 ) * myVarName60_g1459 ));
				float RainDotsRoughness981 = ( saturate( ( tex2D( _GradientTex, panner63_g1459 ).g * step( voroi58_g1459 , (0.0 + (_Size - 0.0) * (0.05 - 0.0) / (1.0 - 0.0)) ) ) ) * _Intensity );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN , bool ase_vface : SV_IsFrontFace ) : SV_TARGET
			{
				...
				float FlipBookTiling100_g1387 = _FlipBTilingNormal;
				float2 temp_cast_3 = (FlipBookTiling100_g1387).xx;
				float2 texCoord114_g1387 = IN.ase_texcoord4.xyz.xy * temp_cast_3 + float2( 0,0 );
				float2 appendResult124_g1387 = (float2(frac( texCoord114_g1387.x ) , frac( texCoord114_g1387.y )));
				float XColumns119_g1387 = _XColumnsYRowsZSpeedWStrartFrameNormal.x;
				float YRows118_g1387 = _XColumnsYRowsZSpeedWStrartFrameNormal.y;
				float ZSpeed121_g1387 = ( ( _XColumnsYRowsZSpeedWStrartFrameNormal.x * _XColumnsYRowsZSpeedWStrartFrameNormal.y ) * _XColumnsYRowsZSpeedWStrartFrameNormal.z );
				...
				float3 tex2DNode132_g1387 = unpack132_g1387;
				float2 temp_cast_4 = (( FlipBookTiling100_g1387 / _ScaleFBDetailsNormal )).xx;
				float2 texCoord105_g1387 = IN.ase_texcoord4.xyz.xy * temp_cast_4 + _OffsetFBDetailsNormal;
				float cos111_g1387 = cos( _FBDetailsNormal );
				float sin111_g1387 = sin( _FBDetailsNormal );
				float2 rotator111_g1387 = mul( texCoord105_g1387 - float2( 0.5,0.5 ) , float2x2( cos111_g1387 , -sin111_g1387 , sin111_g1387 , cos111_g1387 )) + float2( 0.5,0.5 );
				float2 break112_g1387 = rotator111_g1387;
